      ...
      <div>
        ...
        <input
          ...
          className={`w-full px-4 py-3 bg-surface-tertiary border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 transition-colors ${
            errors.name ? 'border-red-400 focus:border-red-500' : 'border-border focus:border-brand-500'
          }`}
          required
        />
        {errors.name && <p className="mt-1.5 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{errors.name}</p>}
      </div>
      <div>
...
hover:text-text-secondary">
      ...
      <div>
        ...
        <input
          ...
          className={`w-full px-4 py-3 bg-surface-tertiary border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 transition-colors ${
            errors.confirmPassword ? 'border-red-400 focus:border-red-500' : 'border-border focus:border-brand-500'
          }`}
          required
          minLength={8}
        />
        {errors.confirmPassword && <p className="mt-1.5 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-3 h-3"
/>{errors.confirmPassword}</p>}
      </div>
      <button type="submit" className="w-full py-3 bg-brand-500 hover:bg-brand-600 text-white font-semibold rounded-xl transition-colors flex items-center
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          required
        />
        {errors.phone && <p className="mt-1.5 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{errors.phone}</p>}
      </div>
      <button type="submit" disabled={loading} className="w-full py-3 bg-brand-500 hover:bg-brand-600 disabled:bg-brand-300 text-white font-semibold rounded-xl
transition-colors flex items-center justify-center gap-2">
        {loading && <Loader2 className="w-4 h-4 animate-spin" />}
        {loading ? 'در حال ایجاد حساب...' : 'ایجاد حساب'}
      </button>
      <button type="button" onClick={() => setStep(1)} className="w-full py-3 text-text-secondary border border-border hover:bg-surface-tertiary font-medium
          ...
          <div className="flex items-center gap-2 mb-1">
            {[1, 2].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${step >= s ? 'bg-brand-500 text-white' :
